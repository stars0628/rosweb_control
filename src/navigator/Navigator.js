...
NAV2D.Navigator = function (options) {
  ...
  function sendGoal(pose) {
    ...
    if (that.goalMarker === null) {
      ...
      //that.rootObject.addChild(that.goalMarker);生成箭头，有bug
    }

    that.rootObject.addChild(that.goalMarker);//移动至函数外
  ...
  //初始化位置
  function set_robot_pose(pose) {
    init_pose = false;
    var myDate3 = new Date();
    var myDate4 = new Date();
    myDate3 = myDate3.getTime(); // 获取当前时间(从1970.1.1开始的毫秒数)
    myDate4 = myDate4.getMilliseconds();//获取当前秒的毫秒数
    init_pose_message.header.stamp.secs = myDate3 / 1000;
    init_pose_message.header.stamp.nsecs = myDate4 * 1000000;
    init_pose_message.pose.pose.position.x = pose.position.x;
    init_pose_message.pose.pose.position.y = pose.position.y;
    init_pose_message.pose.pose.orientation.z = pose.orientation.z;
    init_pose_message.pose.pose.orientation.w = pose.orientation.w;
    init_robot_pose.publish(init_pose_message);
  }
  /**
  ...
  var updateRobotPosition = function (pose, orientation) {
    ...
    if (!initScaleSet) {
      //显示有bug，有时箭头会非常大，注释，修正
      // robotMarker.scaleX = 1.0 / stage.scaleX;
      // robotMarker.scaleY = 1.0 / stage.scaleY;
      robotMarker.scaleX = 1.0 / 50;
      robotMarker.scaleY = 1.0 / 60;
      initScaleSet = true;
    }
    // change the angle
    ...
    var mouseEventHandler = function (event, mouseState) {
      ...
      if (mouseState === 'down') {
        ...
        if (mouseDown === true) {
          ...
          if (init_pose == false)//设置机器人位置时不一样的箭头
          {
            if (use_image && ROS2D.hasOwnProperty('ImageNavigator')) {
            ...
            if (use_image && ROS2D.hasOwnProperty('ImageNavigator')) {
              ...
              orientationMarker = new ROS2D.NavigationArrow({
                size: 10,
                strokeSize: 1,
                fillColor: createjs.Graphics.getRGB(255, 255, 255, 0.66),
                pulse: false
              });
            }
          }
